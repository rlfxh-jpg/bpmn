...
export const isSystemTask = (businessObject: Record<string, unknown> | null | undefined) => {
  const nodeMeta = getNodeMeta(businessObject as RawBusinessObject | undefined)
  return nodeMeta?.nodeKey === 'system-task'
}

export const writeSystemTaskDefaults = (
  moddle: ModdleLike,
  businessObject: Record<string, unknown>
) => {
  const nodeMeta = ensureNodeMeta(moddle, businessObject as RawBusinessObject)
  nodeMeta.nodeKey = 'system-task'

  const fields = [
    moddle.create(CUSTOM_FIELD_TYPE, { key: 'serviceCode', value: '' }),
    moddle.create(CUSTOM_FIELD_TYPE, { key: 'retryPolicy', value: 'none' })
  ]
